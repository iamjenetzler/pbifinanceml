...
Get-PowerBIWorkspace -Scope Organization -All | where {$_.Name -like "user*workspace" -and $_.state -eq "active"} 
...
$userArray = Get-MsolUser -All  | where {$_.DisplayName -like "user*"} 
...
(Get-PowerBIWorkspace -Scope Organization -All | where {$_.Name -like "user*workspace" -and $_.state -eq "active"}) | foreach {
        $Id = $_.Id  
        $Url = "groups/$Id"
